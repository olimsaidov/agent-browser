...
Install `@olimsaidov/agent-browser-wasm` and call `createAgentBrowser` with a CDP `transport`. Optional `cursor: true` shows pointer and click feedback, while `inputMode: "human"` enables curved mouse movement. Commands can also use `--human`, such as `mouse move 240 160 --human` and `drag #source #target --human`. This adapter does not launch browsers or encode recordings; its live cursor is independent of the native recording command. See the [browser package documentation](https://github.com/olimsaidov/agent-browser/tree/main/packages/wasm-core).
...
<tr><td>WebGPU probe</td><td>Opt-in via <code>--webgpu</code>: launches a scratch session with the <a href="/webgpu">WebGPU preset</a>, renders through a real WebGPU pass, and pixel-checks both an offscreen readback and a decoded screenshot. Add <code>--headed</code> to validate the headed/Xvfb capture path</td></tr>
</tbody>
</table>
